...
function predicate_single(
  ...
  g_container.draw = function (
    ...
    let drag_min = d3.drag().on("drag", function (event, d) {
      ...
      d3.select(this).attr("cx", sx.domain([d.vmin, d.vmax])(d.interval_min));
    ...
    let drag_max = d3.drag().on("drag", function (event, d) {
      ...
      d3.select(this).attr("cx", sx.domain([d.vmin, d.vmax])(d.interval_max));
...
function predicate_contrastive(
  ...
  g_container.draw = function (
    ...
    let attribute_text_max_length = d3.max(splom_attributes, (d) => d.length);
    //origin at interval min
...
function predicate_multiple(
      ...
      g.attr(
        ...
        .call(draw_intervals, {
          ...
          intervals: predicates.map((pt) =>
            attr_name in pt ? pt[attr_name] : [undefined, undefined],
          ),
          n_boxes: n_boxes,
        })
  ...
  function draw_images(selected, sy) {
    ...
    let img_selection = g
      ...
      .attr("height", img_size)
      .attr("x", (_, i) => pad + (i % n_cols) * img_size)
      .attr("y", (_, i) => Math.floor(i / n_cols) * img_size + sy.range()[1])
      .attr("href", (d) => d.image_url);
    img_selection.exit().remove();
  }
  ...
  function draw_intervals(
    ...
    let sy = d3
      ...
      .range([height - margin_bottom, margin_top + interval_stroke_width / 2]);
