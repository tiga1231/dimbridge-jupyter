...
import {C, reshape, create_svg, linspace, zip, scatter, overflow_box} from "../lib.js";
...
export function set_selected(data, brushed_region, x, y, cf, crossfilter_dimensions) {
...
export function set_selected_2(data, sample_brush_history, x, y) {
  ...
  data.forEach((d) => {
    ...
    let x0, x1, y0, y1;
    [[x0, x1], [y0, y1]] = [sample_brush_history[0].x_extent, sample_brush_history[0].y_extent];
    d.first_brush = x0 < x(d) && x(d) < x1 && y0 < y(d) && y(d) < y1;
    [[x0, x1], [y0, y1]] = [sample_brush_history[1].x_extent, sample_brush_history[1].y_extent];
...
export function update_brush_history(full_brush_history, brushed_region, n_boxes) {
...
export function get_point_style(mode = "confusion") {
  ...
  if (mode === "confusion") {
    ...
    style = (d, i) =>
      d.brushed ? {fill: d3.schemeCategory10[0], stroke: "#eee"} : {fill: "#aaa", stroke: "#eee"};
