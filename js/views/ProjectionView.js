...
import {make_frame, make_sub_frame, make_bridge_frame} from "../lib/fancy-frames.js";

import {C, reshape, create_svg, linspace, zip, scatter, overflow_box, scatter_gl} from "../lib.js";
...
export default class ProjectionView {
  ...
  */
  ...
  init_crossfilter(data) {
    ...
    let cf_dimensions = cf_attributes.map((attr) => cf.dimension((d) => d[attr]));
    cf_attributes.push("x", "y");
    cf_dimensions.push(
      cf.dimension((d, i) => this.x[i]),
      cf.dimension((d, i) => this.y[i]),
    );
    this.crossfilter_dimensions = Object.fromEntries(zip(cf_attributes, cf_dimensions));
    return cf;
  }

  init_node() {
    let {width, scatter_width, scatter_height, font_size, scatter_padding} = this.config;
  ...
  init_brush() {
    ...
    let plot_extent_y = [this.padding_top, this.plot_height - this.padding_bottom];
  ...
  async brushed(event) {
    ...
    console.assert(event.selection !== null, "[function brushed(event)] event.selection is null. ");
    ...
    if (this.n_boxes > 1) {
      this.g_brush_path.call(draw_path, this.sample_brush_history, {
        ...
        size: this.n_boxes == 2 ? 0 : this.full_brush_history[0].brush_size,
        "stroke-width": 3,
      });
    }
  ...
  async brush_end(event) {
    ...
    if (this.n_boxes > 1 && event.mode === "drag") {
      ...
      if (this.n_boxes == 2) {
        ...
        set_selected_2(this.data, this.sample_brush_history, this.x, this.y);
      }
    }
    //compute predicates based on selected data
    let {predicates, attributes, qualities} = await compute_predicates(
